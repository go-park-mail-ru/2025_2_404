...
CREATE TABLE IF NOT EXISTS client (
	...
	password_hash TEXT NOT NULL CHECK (
		length(password_hash) >= 8 AND length(password_hash) <= 50
	),
...
CREATE TABLE IF NOT EXISTS platform (
	id INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
	platform_name TEXT NOT NULL CHECK (
...
CREATE TABLE IF NOT EXISTS ad (
	id INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
	creator_id INTEGER REFERENCES client(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS statistics (
	id INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
	ad_details_id INTEGER REFERENCES ad_details(id) ON DELETE CASCADE,
	clicks INT NOT NULL DEFAULT 0 CHECK (clicks >= 0),
	impressions INT NOT NULL DEFAULT 0 CHECK (impressions >= 0),
    conversion DECIMAL(5, 2) NOT NULL DEFAULT 0 CHECK (conversion >= 0),
	created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
	updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
);
